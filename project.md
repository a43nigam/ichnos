...
| Dataset ingestion | In progress | Synthetic pose, synthetic Parquet, JSON-pose MCAP, ROS pose MCAP, KITTI raw OXTS, nuScenes ego-pose, and EuRoC groundtruth/IMU/camera ingestion implemented; Python Adapter SDK now wraps existing dataset paths with manifest/CLI dispatch, EuRoC calibration metadata joins, and reviewed generic CSV/Parquet/image-sequence mappings into managed outputs; richer KITTI/nuScenes calibration adapters remain. |
...
4. **M3: Public dataset ingestion**
   ...
   - Status: JSON-pose MCAP, common ROS pose MCAP schemas, KITTI raw OXTS, nuScenes ego-pose, and EuRoC groundtruth/IMU/camera paths complete; Python Adapter SDK supports built-in and entry-point dataset adapters; EuRoC profile/manifest calibration metadata joins are implemented; KITTI/nuScenes/CARLA/Hilti dataset-specific calibration conventions remain.
5. **M4: Demo loop**
...
## Dataset Adapter SDK
- `physicaldb.adapters` defines the Python-first dataset extension point: `DatasetAdapter`, `AdapterRegistry`, `list_adapters()`, `get_adapter()`, and `register_adapter()`.
- Built-in adapters cover EuRoC, normalized Parquet, MCAP pose, KITTI OXTS, nuScenes ego pose, generic dataset profiling, and a final generic media placeholder; external packages can register adapters through the `physicaldb.dataset_adapters` Python entry-point group.
- EuRoC inspection joins optional `mav0/<stream_id>/sensor.yaml` calibration metadata into profiles, manifests, and ingest reports for auditing; transforms are not yet applied during tensor/query materialization.
- `physicaldb.inspect_dataset()`, `suggest_manifest()`, `validate_manifest()`, and `ingest_manifest()` remain backward-compatible and now accept optional adapter routing. Manifests remain JSON v1; `adapter_id`, `adapter_options`, `confidence`, `discovery`, `mapping_status`, and per-stream warnings are additive.
- The `generic_dataset` adapter profiles MCAP, Parquet, CSV/image folders, and S3-compatible prefixes with deterministic hints and review warnings. It emits draft mappings rather than silently finalizing uncertain semantics; explicit invalid final mappings still fail validation. Reviewed final local CSV/Parquet pose+IMU mappings and timestamped camera image sequences now ingest into managed Parquet/catalog outputs with timestamp-unit normalization, derived pose velocity, calibration sidecar discovery, preflighted mappings, and atomic output-root commits, while generic S3 raw ingest remains inspect-only.
- CLI onboarding commands are `robotics dataset adapters`, `robotics dataset inspect --adapter auto ...`, `robotics dataset init-manifest --adapter auto ...`, `robotics dataset init-mapping --adapter generic_dataset ...`, `robotics dataset validate ...`, `robotics dataset ingest ...`, `robotics dataset demo ...`, `robotics dataset query ...`, `robotics dataset stage-s3 ...`, and `robotics dataset upload-managed ...`. S3 prefix inspection/staging uses `robotics object-store list|get|sync-prefix`.
...
- `scripts/run_localstack_s3_smoke.py` starts or reuses a local LocalStack S3 container, creates the configured bucket, sets the same `AWS_*` environment shape used by production S3-compatible paths, and runs `scripts/smoke_s3_pose_imu_media.py` against `s3://...` URIs without committing secrets. Add `--vicon-room1` to run the full three-sequence EuRoC Vicon Room 1 validator through LocalStack S3. It defaults to `localstack/localstack:4.14.0` for token-free local S3 emulation; pass `--image localstack/localstack:latest` only with `LOCALSTACK_AUTH_TOKEN` exported in the shell.
...
- `env CARGO_TARGET_DIR=/tmp/robotics-target PYTHONPATH=python python3 -m pytest tests/test_physicaldb.py`: passing, 32 Python tests with DuckDB 1.5.2, including synchronized EuRoC-style pose+IMU tensor output from catalog URI-driven Rust row-group materialization, persistent DuckDB catalog DB query parity, constrained behavioral predicates, coarse spatial pruning explain diagnostics, enforced pose+IMU/camera cold-read accounting, proof and batch manifests, camera row-group egress accounting/materialization, multi-session `plan_batch()`/`query_batch()` windows, pre-materialization egress blocking, default temporal gap rejection, adapter SDK onboarding, EuRoC calibration metadata, generic CSV/image profiling, draft mapping validation, reviewed generic managed ingest hardening, timestamp-unit normalization, derived generic velocity, customer demo/query CLI workflow, and the `init-mapping` CLI alias.
- `env CARGO_TARGET_DIR=/tmp/robotics-target pytest -q tests/test_physicaldb.py::test_dataset_adapter_registry_and_cli_list_adapters tests/test_physicaldb.py::test_dataset_adapter_unknown_id_error_is_actionable tests/test_physicaldb.py::test_dataset_adapter_entry_point_discovery tests/test_physicaldb.py::test_dataset_adapter_auto_detects_existing_adapter_fixtures tests/test_physicaldb.py::test_dataset_cli_inspect_init_validate_euroc_fixture tests/test_physicaldb.py::test_onboarding_validation_rejects_bad_manifests tests/test_physicaldb.py::test_trust_check_helpers_are_deterministic tests/test_physicaldb.py::test_onboarding_inspect_suggest_and_validate_normalized_parquet tests/test_physicaldb.py::test_query_returns_numpy_tensor`: passing, covering Adapter SDK registry/entry-point discovery, CLI onboarding, legacy manifest validation, tensor certificates, and normalized Parquet inspection.
- `env CARGO_TARGET_DIR=/tmp/robotics-target /tmp/robotics-target/debug/robotics dataset adapters`: passing, listing built-in dataset adapters.
- `env CARGO_TARGET_DIR=/tmp/robotics-target /tmp/robotics-target/debug/robotics dataset inspect --adapter auto --input /tmp/robotics_adapter_smoke/euroc --out /tmp/robotics_adapter_smoke/profile.json`, followed by `dataset init-manifest --adapter auto` and `dataset validate`: passing on a tiny EuRoC-style fixture.
...
- `env PYTHONPATH=python CARGO_TARGET_DIR=/tmp/robotics-target python3 scripts/run_localstack_s3_smoke.py`: pulled and started `localstack/localstack:4.14.0`, created `s3://robotics`, uploaded pose/IMU/media Parquet objects to LocalStack S3, and passed the compact enforced cold-read smoke with tensor shape `[3, 9]`, pose/IMU/media matched row groups `2/2/2`, selected bytes `1184/882/1184`, total selected bytes `3250`, planned materialized read bytes `2066`, actual cold reads `50`, actual cold-read bytes `9072`, footer bytes `7006`, range audit/enforcement passed, and low-budget media egress blocked before materialization.
- `env PYTHONPATH=python CARGO_TARGET_DIR=/tmp/robotics-target python3 scripts/run_localstack_s3_smoke.py --vicon-room1 --vicon-output-root data/validation/euroc_vicon_room1_s3 --vicon-iterations 5`: compact LocalStack S3 smoke passed, then the full EuRoC Vicon Room 1 S3 run ingested local real zips and uploaded representative pose/IMU/cam0 objects through `s3://robotics/euroc-vicon-room1/...`; the first run exposed hidden CLI stderr, the second run reached the third camera upload before Docker Desktop stopped serving the daemon. The script now forwards failed Rust CLI stdout/stderr and retries S3 uploads at the harness layer so transient LocalStack PUT failures are debuggable and retried.
...
- EuRoC ingestion currently covers groundtruth pose, IMU CSV tables, camera byte extraction, and adapter-level calibration metadata joins; applying transforms during tensor/query materialization is not yet modeled.
...
- Adapter SDK entry-point discovery is implemented, but third-party adapter packaging should be validated with a real external wheel before treating the plugin contract as stable.
