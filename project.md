...
- `scripts/run_localstack_s3_smoke.py` starts or reuses a local LocalStack S3 container, creates the configured bucket, sets the same `AWS_*` environment shape used by production S3-compatible paths, and runs `scripts/smoke_s3_pose_imu_media.py` against `s3://...` URIs without committing secrets. It defaults to `localstack/localstack:4.14.0` for token-free local S3 emulation; pass `--image localstack/localstack:latest` only with `LOCALSTACK_AUTH_TOKEN` exported in the shell.
